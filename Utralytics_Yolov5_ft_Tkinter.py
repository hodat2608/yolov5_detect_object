...
class Model_Camera_1(PLC_Connection,MySQL_Connection):
    ...
    # Hàm gọi ngoài class nên cần custom riêng các thông số khác so với các hàm được gọi trong class
    def processing_handle_image_customize_backup(self,input_image,width,height):
        label_remove = []
        ...
        for i in range(len(table_results.index)):
            ...
            for i1 in range(len(model_names)):
                for setting in model_settings:
                    print(setting['label_name'],setting['width_min'] ,setting['width_max'] )
    ...
    def processing_handle_image_customize(self, input_image, width, height):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(check_label, i) for i in range(len(table_results.index))]
        ...
        for model_name in model_names:
            for setting in model_settings:
                if model_name == setting['label_name']:
                    if setting['join_detect']:
                        if setting['OK_jont']:
                            number_of_labels = name_rest.count(model_name)
                            if number_of_labels != setting['num_labels']:
                                results_detect = 'NG'
                                ok_variable = True
                                list_label_ng.append(model_name)
                        if setting['NG_jont']:
                            if model_name in name_rest:
                                results_detect = 'NG'
                                ok_variable = True
                                list_label_ng.append(model_name)
        ...
        for i1 in range(len(model1.names)):
            for record in records:
                if record[4] == model1.names[i1]:
                    print(self.join[i1],bool(record[5]))
...
class Model_Camera_2(PLC_Connection,MySQL_Connection):
    ...
    def mohica(self):
        ...
        filename = "C:/FH/New folder (3)/2024-05-21_15-50-51-289134_luu_xuat.jpg"
        shutil.copy(filename,filepath)
    ...
    def update_images(self,window, camera2_frame):
        self.display_images_c2(camera2_frame,2)
        window.after(50, self.update_images,window,camera2_frame)
    ...
    def Display_Camera(self,tab2):
        # tab1 = ttk.Frame(notebook)
        # notebook.add(tab1, text="Display Camera")
        camera2_frame = self.create_camera_frame_cam2(tab2, 2)   
        return camera2_frame
    ...
    def option_layout_models(self, Frame_1, Frame_2,records):
        ...
        self.scale_conf_all.grid(row=4, column=0, columnspan=2, padx=30, pady=5, sticky="nws")
        self.lockable_widgets.append(self.scale_conf_all)
        scale_conf_all = self.scale_conf_all
...
class Main_Display():
    ...
    def __init__(self,window,notebook):
        ...
        self.handle_image = Model_Camera_1()
    ...
    def display_images_c1(self, camera_frame, camera_number):
        ...
        if len(image_paths) == 0:
            ...
            for filename in image_paths:
                ...
                for widget in camera_frame.winfo_children():
                    widget.destroy()
                image_result,time_processing,results_detect,list_label_ng = self.handle_image.processing_handle_image_customize(img1_orgin,width,height,self.model)
    ...
    def display_images_c2(self, camera_frame, camera_number):
        ...
        if len(image_paths) == 0:
            ...
            for filename in image_paths:
                ...
                time_processing_output_2.config(text=f'{time_processing}')
                if results_detect == 'OK':
                    result_detection_2.config(text=results_detect,fg='green')
                else:
                    result_detection_2.config(text=results_detect,fg='red')
                list_label_ng = ','.join(list_label_ng)
                img_pil = Image.fromarray(image_result)
                photo = ImageTk.PhotoImage(img_pil)
                canvas = tk.Canvas(camera_frame, width=width, height=height)
