...
SCALE_X_CAM1 = 1280/2048
SCALE_Y_CAM1 = 960/1536
...
# def load_all():
#     values_all = []
#     with open('static/dimension' + '.txt','r') as lines:
#         for line in lines:
#             _, name_all = line.strip().split('=')
#             values_all.append(name_all)
#     window['my_width'].update(value=values_all[0])
#     window['my_height'].update(value=values_all[1])
    # end
...
# def save_all():
#     with open('static/dimension' + '.txt','w') as f:
#         f.write('width' + '=' + str(values['my_width']))
#         f.write('\n')
#         f.write('height' + '=' + str(values['my_height']))
#         f.write('\n')
...
def program_camera1(model,size,conf):
    ...
    read_2000 = readdata('DM10000') 
    read_30000 = readdata('DM30000') 
    read_30002 = readdata('DM30002') 
    read_30004 = readdata('DM30004') 



    if read_2000 == 1:# and (read_30000==10 or read_30002==10): 
        # busy
        writedata('DM20030.U',10) 

        img1_orgin = my_callback1.image 
        img1_save = img1_orgin.copy()
        img1_orgin = cv2.cvtColor(img1_orgin, cv2.COLOR_BGR2RGB)
        img1_orgin = cv2.rotate(img1_orgin, cv2.ROTATE_180)
        demsolan +=1
        print('Start')
        print('so lan chup ', demsolan)
        # print(read_30004)
        # print(read_30000, ' ',read_30002,' ', read_30004)

        t1 = time.time()        
        writedata('DM10000.U',0) 
        writedata('DM30000.U',0) 
        ...
        for item in range(len(table1.index)):
            ...
            if table1['name'][item] =="1" and conf1 >int(values['1_Conf_1']):
                center_x1 = int((float(table1['xmin'][item]) + float(width1)/2))
                center_y1= int((float(table1['ymin'][item]) + float(height1)/2))
                ha_center1 = 1
            if table1['name'][item] =="2" and conf1 >int(values['2_Conf_1']):
                center_x2 = int((float(table1['xmin'][item]) + float(width1)/2))
                center_y2= int((float(table1['ymin'][item]) + float(height1)/2))
                ha_center2 = 1
        ...
        # my_width = int(values['my_width'])
        # my_height = int(values['my_height'])

        # cv2.putText(show1, '*',(my_width-5,my_height+5),cv2.FONT_HERSHEY_COMPLEX, 1,(0,0,255),1)
        # cv2.putText(show1, '+',(center_x,center_y),cv2.FONT_HERSHEY_COMPLEX, 1,(0,0,255),1)


        #cv2.circle(show1,(my_width,my_height), 4,(255,0,0),-1)
        # if ha_center1 == 1 and ha_center2 == 1:
        #     cv2.circle(show1,(center_x1,center_y1), 4,(255,0,0),-1)
        #     cv2.circle(show1,(center_x2,center_y2), 4,(255,0,0),-1)

        #     angle = atan2(center_x1-center_x2,center_y1-center_y2)
        #     angle = round(float(degrees(angle)) + setmyangle,2)
        #     if angle < 0:
        #         angle =360 - abs(angle) 
        #     myangle = angle
        #     angle = int(angle*100)
        #     writedata('DM20030.U',0) 
        #     if center_x1 != 0 and center_y1 != 0 and center_x2 != 0 and center_y2 != 0:
        #         cv2.putText(show1, f'{myangle}',(40,40),cv2.FONT_HERSHEY_COMPLEX, 1,(255,255,255),1)
        #         writedata('DM20020.U',angle)
        #         writedata('DM10005.U',5) 

        #     else:
        #         # loi
        #         cv2.putText(show1, 'No detect',(40,40),cv2.FONT_HERSHEY_COMPLEX, 1,(255,255,255),1)
        #         writedata('DM10005.U',10) 
        # else:
        #     # writedata('DM2030.U',0) 
        #     writedata('DM10005.U',10) 
        ...
        #cv2.circle(show1,(my_width,my_height), 4,(255,0,0),-1)
        #cv2.circle(show1,(center_x,center_y), 4,(255,0,0),-1)
        # if ha_center1 == 1 and ha_center2 == 1:
        ...
        #     angle = atan2(center_x1-center_x2,center_y1-center_y2)
        #     #angle = atan2(my_width-center_x,my_height-center_y)
        #     angle = round(float(degrees(angle) - 180),2)
            
        #     if angle < 0:
        #         angle =360 - abs(angle)
        #     if center_x1 != 0 and center_y1 != 0 and center_x2 != 0 and center_y2 != 0:
        #         cv2.putText(show1, f'{angle}',(40,40),cv2.FONT_HERSHEY_COMPLEX, 2,(255,255,255),1)
        #     else:
        #         cv2.putText(show1, 'No detect',(40,40),cv2.FONT_HERSHEY_COMPLEX, 2,(255,255,255),1)
        # else:
        #     cv2.putText(show1, 'No detect',(40,40),cv2.FONT_HERSHEY_COMPLEX, 2,(255,255,255),1)
    ...
    layout_main = [
        ...
        sg.Frame('',[
            ...
            sg.Frame('',[
                ...
                [sg.Text('')],
                #[sg.Text('Width:  '),sg.Slider(range=(1,1280),default_value=598,orientation='h',size=(30,20),font=('Helvetica',11), key= 'my_width')],
                # [sg.Text('Height:  '),sg.Slider(range=(1,960),default_value=518,orientation='h',size=(30,20),font=('Helvetica',11), key= 'my_height')],
                # [sg.Text('')],
...
while connected == False:
    connected = socket_connect('192.168.1.1',8501)
print("connected") 
writedata('DM20020.U',0)

#load_all()
writedata('DM20050.U',10) 
...
while True:
                ...
                if row1_a == '1':
                    for item in range(len(model1.names)):
                        if int(row1_b) == item:
                            ...
                            window['PLC_OK_1'].update(value=str(row[17]))
    ...
    program_camera1(model=model1,size= 608,conf= values['conf_thres1']/100)
    # program_camera1_FH(model=model1,size= 608,conf= values['conf_thres1']/100)




    #task_camera1(model=model1,size= 608,conf= values['conf_thres1']/100)


    #test_camera1(model=model1,size= 608,conf= values['conf_thres1']/100)


    #task1(model1,size= 608,conf= values['conf_thres1']/100)
    ...
    # task_thread1 = threading.Thread(target=program_camera1_FH_test, args=(model1, 608, values['conf_thres1']/100,))
    ...
    # task_thread1 = multiprocessing.Process(target=program_camera1_FH_test, args=(model1, 608, values['conf_thres1']/100,))
    ...
    if recording1:
        ...
        #img1_orgin = img1_orgin[50:530,70:710]
        img1_orgin = img1_orgin.copy()
        img1_orgins = cv2.rotate(img1_orgin, cv2.ROTATE_180)
        ...
        for item in range(len(table1.index)):
            ...
            height1 = table1['ymax'][item] - table1['ymin'][item]
            conf1 = table1['confidence'][item] *100
            if table1['name'][item] =="1":
                center_x1 = int((float(table1['xmin'][item]) + float(width1)/2))
                center_y1= int((float(table1['ymin'][item]) + float(height1)/2))
                ha_center1 = 1
            if table1['name'][item] =="2":
                center_x2 = int((float(table1['xmin'][item]) + float(width1)/2))
                center_y2= int((float(table1['ymin'][item]) + float(height1)/2))
                ha_center2 = 1
                
            
            #center_x = int((float(table1['xmin'][item]) + float(width1)/2))
            #center_y = int((float(table1['ymin'][item]) + float(height1)/2))
            #print(center_x)
            #area1 = width1*height1
            label_name = table1['name'][item]
            for i1 in range(len(model1.names)):
        ...
        if ha_center1 == 1 and ha_center2 == 1:
            ...
            if angle < 0:
                angle =360 - abs(angle) 
            if center_x1 != 0  and center_y1 != 0 and center_x2 != 0  and center_y2 != 0:
                cv2.putText(show1, f'{angle}',(40,40),cv2.FONT_HERSHEY_COMPLEX, 1,(255,255,255),1)
            else:
                cv2.putText(show1, 'No detect',(40,40),cv2.FONT_HERSHEY_COMPLEX, 1,(255,255,255),1)
    ...
    if event == 'Snap1':
        ...
        show1 = cv2.resize(show1, (image_width_display,image_height_display), interpolation = cv2.INTER_AREA)
        show1 = cv2.cvtColor(show1, cv2.COLOR_BGR2RGB)
        name_file = time_to_name()
        # cv2.imwrite("D:/a"  + '/' + name_file + '.jpg',img1_save)
