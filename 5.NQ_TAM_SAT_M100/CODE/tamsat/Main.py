...
from myutils import run_model, load_choosemodel, load_theme, make_window, load_all_sql, save_data, remove_file,write_plc,read_plc
from myutils import update_disabled_control, change_disabled_administrator, add_file_browser, layout_theme, choose_model_event
from myutils import check_model, camera, detect_model, picture, set_param_before_choose_model_cam, row_for_var, var_for_interface
from myutils import program_camera_FH_c4,update_value_model, login_administrator,change_model,load_kc,detect_model_4,program_camera_FH,run_plc_keyence
import PySimpleGUI as sg
import traceback
from myutils import read_plc, write_plc, excel_ppbydate, excel_hangmuc, excel_ray, excel_handle_pphangmuc, excel_handle_ppray, excel_handle_all
from myutils import collect_dict_date,logging, CMyCallback, setup_camera_stc,program_camera, kiem_guong
from myutils import config_off_auto, set_config_init_camera, program_camera_test
...
try:
    while True:
        ...
        #moplc
        try:
            excel_ray(plc_name, 6060,'Ngay')
            excel_ray(plc_name, 6062,'Dem')
            excel_hangmuc(plc_name, 6050,'Ngay')
            excel_hangmuc(plc_name, 6052,'Dem')
            excel_handle_pphangmuc(plc_name, 1110)
            excel_handle_ppray(plc_name, 1112)
        ...
        try:
            if read_plc(plc_name,4180) == 1:
                for var_plc in range(0,5):
                    # tu 4100 den 4104
                    if read_plc(plc_name,4100 + var_plc) == 1:
                        excel_ppbydate(var_plc,"CAM 1")
                        write_plc(plc_name,4100 + var_plc,0) 
