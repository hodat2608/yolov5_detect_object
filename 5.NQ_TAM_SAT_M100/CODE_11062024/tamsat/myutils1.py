...
def handle_image(img_orgin, model,size,conf,num_camera,values, kiem_guong=False,center_x_min = 0, center_x_max =0,center_y_min =0, center_y_max=0):
    ...
    for item in range(len(table.index)):
        ...
        if ck==0:
            if '2' == label_name:
                list5.append([int(table['ymin'][item]),int(table['ymax'][item])])
            if label_name == '3':
                list6.append([int(table['ymin'][item]),int(table['ymax'][item])])
    ...
    if list5 != [] and list6 != []:
        lech_tren =list6[0][0] - list5[0][0]
        if lech_tren <=10:
            ng_kck=1
            if num_camera ==1:
                bien_ket_qua_1.append(20204)
            if num_camera ==2:
                bien_ket_qua_2.append(20304)

            cv2.putText(show, f'Lech tren: {lech_tren}',(50,50),cv2.FONT_HERSHEY_COMPLEX, 1,(255,0,0),2)
        else:
            cv2.putText(show, f'Lech tren: {lech_tren}',(50,50),cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,0),2)

        lech_duoi =list5[0][1] - list6[0][1]
        if lech_duoi <=10:
            ng_kck=1
            ...
            if num_camera ==2:
                bien_ket_qua_2.append(20304)
            cv2.putText(show, f'Lech duoi: {lech_duoi}',(50,100),cv2.FONT_HERSHEY_COMPLEX, 1,(255,0,0),2)
            
        else:
            cv2.putText(show, f'Lech duoi: {lech_duoi}',(50,100),cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,0),2)
...
def show_and_save(window, result,show,num_camera,img_orgin,values,okng=True):
    ...
    if values[f'have_save_{result}_{num_camera}']:
        ...
        if not os.path.isdir(f'D:/cam{num_camera}/'+ "/" + str(today)):
            os.mkdir(f'D:/cam{num_camera}/'+ "/" + str(today))
        cv2.imwrite(f'D:/cam{num_camera}/'+ "/" + str(today) + '/'+ name_folder + '.jpg',show)
    ...
    if num_camera == 1:
        layout_main = [
            ...
            sg.Frame('',[
                ...
                [sg.Frame('',[
                    [sg.Text(' '),sg.Button('Pic', size=(size_x,1), font=('Helvetica',font_size),disabled=True,key= f'Pic{num_camera}'),
                    # [sg.Text('')],
                    sg.Text(' '),sg.Button('Detect', size=(size_x,1), font=('Helvetica',font_size),disabled=True,key= f'Detect{num_camera}'),
...
def layout_option(model, num_camera):
    ...
    layout_option = [
        ...
        [   
            ...
            [sg.Text('Weights', size=(12,1), font=('Helvetica',15),text_color='red'), sg.Input(size=(60,1), font=('Helvetica',12), key=f'file_weights{num_camera}',readonly= True, text_color='navy',enable_events= True),
            sg.Frame('',[
                [sg.FileBrowse(file_types= file_weights, size=(12,1), font=('Helvetica',10),key= f'file_browse{num_camera}',enable_events=True, disabled=True)]
            ], relief= sg.RELIEF_FLAT),
            sg.Frame('',[
                [sg.Button('Change Model', size=(14,1), font=('Helvetica',10), disabled= True, key= f'Change_{num_camera}')]
            ], relief= sg.RELIEF_FLAT),],
            [sg.Text('Confidence',size=(12,1),font=('Helvetica',15), text_color='red'), sg.Slider(range=(1,100),orientation='h',size=(60,20),font=('Helvetica',11),disabled=True, key= f'conf_thres{num_camera}'),
            sg.Text('Size',size=(12,1),font=('Helvetica',15), text_color='red'),
            sg.InputCombo((416,512,608,768,896,1024,1280,1408,1536),size=(10,200),font=('Helvetica',11),disabled=True,text_color='navy',default_value=416,key=f'choose_size{num_camera}',)]
        ], relief=sg.RELIEF_FLAT),
        ],
        [sg.Frame('',[
            [sg.Text('Name',size=(15,1),font=('Helvetica',15), text_color='red'), 
        ...
        [sg.Frame('',[
            [
                sg.Text(f'{model.names[i]}_{num_camera}',size=(15,1),font=('Helvetica',15), text_color='yellow'), 
                sg.Checkbox('',size=(5,5),default=True,font=('Helvetica',15),  key=f'{model.names[i]}_{num_camera}',enable_events=True, disabled=True), 
                sg.Checkbox('',size=(5,5),font=('Helvetica',15),  key=f'{model.names[i]}_OK_{num_camera}',enable_events=True, disabled=True), 
                sg.Input('1',size=(2,1),font=('Helvetica',15),key= f'{model.names[i]}_Num_{num_camera}',text_color='navy',enable_events=True, disabled=True), 
                sg.Text('',size=(4,1),font=('Helvetica',15), text_color='red'), 
                sg.Checkbox('',size=(5,5),font=('Helvetica',15),  key=f'{model.names[i]}_NG_{num_camera}',enable_events=True, disabled=True), 
                sg.Input('0',size=(8,1),font=('Helvetica',15),key= f'{model.names[i]}_Wn_{num_camera}',text_color='navy',enable_events=True, disabled=True), 
                sg.Text('',size=(2,1),font=('Helvetica',15), text_color='red'), 
                sg.Input('100000',size=(8,1),font=('Helvetica',15),key= f'{model.names[i]}_Wx_{num_camera}',text_color='navy',enable_events=True, disabled=True), 
                sg.Text('',size=(2,1),font=('Helvetica',15), text_color='red'), 
                sg.Input('0',size=(8,1),font=('Helvetica',15),key= f'{model.names[i]}_Hn_{num_camera}',text_color='navy',enable_events=True, disabled=True), 
                sg.Text('',size=(2,1),font=('Helvetica',15), text_color='red'), 
                sg.Input('100000',size=(8,1),font=('Helvetica',15),key= f'{model.names[i]}_Hx_{num_camera}',text_color='navy',enable_events=True, disabled=True), 
                sg.Text('',size=(2,1),font=('Helvetica',15), text_color='red'), 
                sg.Input('4000',size=(8,1),font=('Helvetica',15),key= f'{model.names[i]}_PLC_{num_camera}',text_color='navy',enable_events=True, disabled=True), 
                sg.Text('',size=(2,1),font=('Helvetica',15), text_color='red'), 
                sg.Slider(range=(1,100),default_value=25,orientation='h',size=(30,20),font=('Helvetica',11), key= f'{model.names[i]}_Conf_{num_camera}', disabled=True),            
            ] for i in range(len(model.names))
        ], relief=sg.RELIEF_FLAT)],
        [sg.Text('  OK',size=(15,1),font=('Helvetica',15), text_color='yellow'),
        sg.Text(' '*230), 
        sg.Input('4000',size=(8,1),font=('Helvetica',15),key= f'PLC_OK_{num_camera}',text_color='navy',enable_events=True, disabled=True)],
        [sg.Text(' ')],
        [sg.Text(' '*250), sg.Button('Save Data', size=(12,1),  font=('Helvetica',12),key=f'SaveData{num_camera}',enable_events=True, disabled=True)] 
        ], relief= sg.RELIEF_FLAT)]
    ]
...
def layout_part_saveimg(status,num_camera):
    layout_part_saveimg = [
        [sg.Text(f'Have save folder image {status} for camera {num_camera}',size=(35,1),font=('Helvetica',12), text_color='yellow'),
         sg.Checkbox('',size=(5,5),default=False,font=('Helvetica',12),  key=f'have_save_{status}_{num_camera}',enable_events=True, disabled=True)], 
...
def change_disabled_administrator(window,event, values, num_camera, model):
    ...
    #     num_camera =2
        ...
        window[f'Detect{num_camera}'].update(disabled= False,button_color='turquoise')
...
def excel_ppbydate(var_plc,name):
    ...
    phe_pham = ["KEO DINH TRUC KEO DINH COMI", "CHAM KEO IT CHAM KEO KHONG DEU","PP KHAC"]
    all_cam = [name]
    # all_cam = []
    tt =0
    # if name == 'CAM 1':
