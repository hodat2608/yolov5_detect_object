...
mypath1 = r"C:\\combine\\model\\Model1\\X75-TRANG-C1-20220924.pt"
...
#3
    ...
    if event == 'start2':
        #nhan = {'cd_trai':3,'tc_tren':4,'cd_phai':3,'tc_duoi':4}
        nhan = {'ok1':5,'ok2':6,'ok3':7}
	    #nhan={'cd_trai':3,'tc_tren':4,'cd_phai':3,'tc_duoi':4}
        if (values['input_weight2'] != '') & (values['input_image2'] !='') & (values['input_save2'] != '') & (values['input_classes2'] != ''):
            mypath = values['input_weight2']
            model = torch.hub.load('./levu','custom', path= mypath, source='local',force_reload =True)
            ...
            for path in glob.glob(mydir):
                ...
                f = open(mysave + name + '.txt', "a") #append label
                for detect in result.xywhn:
                    ...
                    for item in range(len(mydetects)):
                        ...
                        f.write(str(nhan[name_label]) + " " + str(mydetect[0]) + " " + str(mydetect[1]) + " " + str(mydetect[2]) + " " + str(mydetect[3]))
                        f.write("\n")
                    f.close()
...
#6
    ...
    if event == 'start5':
        if values['input_classes5'] != '':
            ...
            program_dir5 = [ dir_py5, ' --img ', '416', ' --batch ', '1' ,' --epochs ', '{}'.format(int(values['input_epoch5'])) , ' --data ', dir_data5 , ' --cfg ', dir_model5, ' --weights ', '""', ' --name ', 'my_results' + '{}'.format(name_folder),  ' --cache']
