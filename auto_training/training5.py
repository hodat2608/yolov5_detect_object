...
#6
    ...
    if event == 'start5':
        if values['input_classes5'] != '' and values['input_name5'] != '' and values['input_save5'] != '':
            ...
            program_dir5 = [ dir_py5, ' --img ', str(values['imgsz5']), ' --batch ', '1' ,' --epochs ', '{}'.format(int(values['input_epoch5'])) , ' --data ', dir_data5 , ' --cfg ', dir_model5, ' --weights ', '""', ' --name ', 'my_results' + '{}'.format(name_folder),  ' --cache']
...
#8
    ...
    if start7 == 1:
        if values['input_classes7'] != '' and values['input_move7'] != '' and values['input_save7'] != '':
            ...
            dir_model5 = os.path.join(os.getcwd()  + '/levu/models/' , 'levu.yaml')
            name_folder = time_to_name()
            program_dir5 = [ dir_py5, ' --img ', str(values['imgsz7']), ' --batch ', '1' ,' --epochs ', '{}'.format(int(values['input_epoch7'])) , ' --data ', dir_data5 , ' --cfg ', dir_model5, ' --weights ', '""', ' --name ', 'my_results' + '{}'.format(name_folder),  ' --cache']
